...
public class EICallableLookupElement extends LiveTemplateLookupElementImpl {

    private final EITypeToken type;
    private final boolean forScript;

    EICallableLookupElement(@NotNull String name,
                            @NotNull List<EIFormalParameter> params,
                            @NotNull EITypeToken returnType,
                            boolean isScript) {
        super(constructTemplate(name, params), true);
        type = returnType;
        forScript = isScript;
    }
    ...
    private static TemplateImpl constructTemplate(@NotNull String name, @NotNull List<EIFormalParameter> params) {
        ...
        for (EIFormalParameter parameter : params) {
            template.addVariable(
                    ...
                    new EIParamExpression(parameter),
                    true
            );
        }
